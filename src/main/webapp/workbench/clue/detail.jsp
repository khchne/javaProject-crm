...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            showClueActivityList();
            $("#activityRelationSearch").on("keydown", function (event) {
                if (event.keyCode === 13) {
                    $.ajax({
                        url: "workbench/clue/activityRelationSearch.do",
                        type: "post",
                        dataType: "json",
                        data: {
                            "activityName": $("#activityRelationSearch").val(),
                            "clueId": "${clue.id}"
                        },
                        success: function (data) {
                            ...
                            $.each(data, function (index, ele) {
                                html += '<tr>';
                                html += '<td><input type="checkbox" value="' + ele.id + '" name="saId"/></td>';
                                html += '<td>' + ele.name + '</td>';
                            ...
                            $("#clueActivityListBody").html(html);
            ...
            $("#allSaId").on("click", function () {
                $("input[name=saId]").prop("checked", this.checked);
            })

            $("#clueActivityListBody").on("click", $("input[name=saId]"), function () {
                $("#allSaId").prop("checked", $("input[name=saId]").length === $("input[name=saId]:checked").length);
            })

            $("#boundBtn").on("click", function () {
                var parameter = "clueId=${clue.id}&";
                var aids = $("input[name=saId]:checked");
                if (aids.length === 0) {
                    alert("请选择需要关联的市场活动");
                } else {
                    for (var i = 0; i < aids.length; i++) {
                        parameter += "activityId=" + aids[i].value;
                        if (i !== aids.length - 1) {
                            parameter += "&";
                        }
                    }
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.success) {
                                ...
                                showClueActivityList();
                                //    清除模态窗口中的内容
                                $("#activityRelationSearch").val("");
                                $("#clueActivityListBody").html("");
                                //    关闭模态窗口
        ...
        function showClueActivityList() {
            $.ajax({
                url: "workbench/clue/getClueActivity.do",
                type: "post",
                dataType: "json",
                data: {
                    "clueId": "${clue.id}"
                ...
                success: function (data) {
                    ...
                    $.each(data, function (index, ele) {
                        ...
                        html += '<td>' + ele.endDate + '</td>';
                        html += '<td>' + ele.owner + '</td>';
                        html += '<td><a href="javascript:void(0);" onclick="unbound(\'' + ele.id + '\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
                        html += '</tr>';
                    })
                    $("#clue-ActivityBody").html(html);
                }
            })
        }
            ...
            <div class="modal-body">
                <div class="btn-group" style="position: relative; top: 18%; left: 8px;">
                    <form class="form-inline" role="form">
                        <div class="form-group has-feedback">
                            <input type="text" class="form-control" id="activityRelationSearch" style="width: 300px;"
                ...
                <table id="activityTable" class="table table-hover" style="width: 900px; position: relative;top: 10px;">
                    ...
                    <tr style="color: #B3B3B3;">
                        <td><input type="checkbox" id="allSaId"/></td>
                        <td>名称</td>
                        <td>开始日期</td>
                        <td>结束日期</td>
                        <td>所有者</td>
