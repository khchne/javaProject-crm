...
<%@ page import="com.example.crm.workbench.domain.Transaction" %>
<%@ page import="com.fasterxml.jackson.databind.ObjectMapper" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%
    List<DicValue> dicValueList = (List<DicValue>) application.getAttribute("stage");
    String json = (String) application.getAttribute("possibility");
    Map map = new ObjectMapper().readValue(json, Map.class);
    int currentIndex = 0;
    String currentStage = ((Transaction) request.getAttribute("tran")).getStage();
    for (int i = 0; i < dicValueList.size(); i++) {
        if (dicValueList.get(i).getText().equals(currentStage)) {
            currentIndex = i;
            break;
        }
    }
...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#possibility").html(${possibility}["${tran.stage}"]);
        ...
        function changeStage(stage, i) {
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        $("#stage").html(stage);
                        $("#possibility").html(${possibility}[stage]);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data, function (index, ele) {
                        var stage = ele.stage;
                        var possibility = ${possibility}[stage];
                        html += '<tr>';
        ...
        function updateIcon(stage, i) {
            var currentPoint = parseInt(i);
            var point = "<%=point%>";
            var len = "<%=dicValueList.size()%>";
            if (currentPoint < point) {
                for (var j = 0; j < currentPoint; j++) {
                    // 绿 圈
                    $("#" + j).removeClass();
                ...
                for (var j = currentPoint + 1; j < point; j++) {
                    // 黑圈
                    $("#" + j).removeClass();
...
<div style="position: relative; left: 40px; top: -50px;">
    ...
    <%
        if (currentIndex < point) {
            for (int i = 0; i < currentIndex; i++) {
                // 全部 全 勾 绿
    %>
    <span class="glyphicon glyphicon-ok-circle mystage" id="<%=i%>"
          onclick="changeStage('<%=dicValueList.get(i).getText()%>','<%=i%>')" data-toggle="popover"
          data-placement="bottom"
          data-content="<%=dicValueList.get(i).getText()%>" style="color: #90F790;"></span>
    -----------
    <%
        }
        // 当前
    %>
    <span class="glyphicon glyphicon-map-marker mystage" id="<%=currentIndex%>" data-toggle="popover"
          data-placement="bottom"
          onclick="changeStage('<%=dicValueList.get(currentIndex).getText()%>','<%=currentIndex%>')"
          data-content="<%=dicValueList.get(currentIndex).getText()%>" style="color: #90F790;"></span>
    -----------
    <%
        // 后面
        for (int i = currentIndex + 1; i < point; i++) {
            // 黑圈
    %>
    <span class="glyphicon glyphicon-record mystage" id="<%=i%>"
          onclick="changeStage('<%=dicValueList.get(i).getText()%>','<%=i%>')" data-toggle="popover"
          data-placement="bottom"
          data-content="<%=dicValueList.get(i).getText()%>"></span>
    -----------
    <%
        }
        for (int i = point; i < dicValueList.size(); i++) {
            // 黑叉
    %>
    <span class="glyphicon glyphicon-remove mystage" id="<%=i%>"
          onclick="changeStage('<%=dicValueList.get(i).getText()%>','<%=i%>')" data-toggle="popover"
          data-placement="bottom"
          data-content="<%=dicValueList.get(i).getText()%>"></span>
    -----------
    ...
    %>
    <span class="glyphicon glyphicon-remove mystage" id="<%=i%>"
          onclick="changeStage('<%=dicValueList.get(i).getText()%>','<%=i%>')" style="color: red;" data-toggle="popover"
          data-placement="bottom"
          data-content="<%=dicValueList.get(i).getText()%>"></span>
    -----------
    <%
